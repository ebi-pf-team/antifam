...
Calculates jaccard distance of two clusters

$distance = $cluster1->jaccard($cluster2);
...
sub jaccard {
  ...
  foreach my $element ($cluster1->each()){
    $count{$element}++;
  }

  foreach my $element ($cluster2->each()){
    $count{$element}++;
  }
  ...
  foreach my $id (keys %count){
    ...
    if ($count{$id}>1){
      $numerator++;
      #warn "found $id in both clusters";
    } else {
      #warn "found $id in one cluster";
    }
  }

  my $distance=1-($numerator/$denominator);
  return $distance;
}
